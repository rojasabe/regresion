...
Un detalle importante al implementar gradiente descendiente manualmente es que el tama\~no de los pasos depende no solo de $\alpha$ sino tambi\'en de la magnitud de los $x^{(i)}$, porque $\partial J/\partial w$ incluye el factor $x^{(i)}$ en cada t\'ermino. Si los valores de entrada son grandes, los gradientes se agrandan proporcionalmente y el algoritmo puede divergir incluso con una tasa que parece razonable. La condici\'on te\'orica de convergencia es $\alpha < 2/\mathbb{E}[x^2]$ \cite{hastie2009elements}.

Los datasets utilizados en este trabajo tienen $x \in [1,7]$, para los cuales el valor m\'aximo seguro es aproximadamente $\alpha \approx 0.08$. Todos los experimentos usan $\alpha \leq 0.01$, con margen suficiente. En la analog\'ia: si la mano que gira la perilla es muy brusca ($\alpha$ alto), la estaci\'on se pasa siempre de largo.

\subsection{Versi\'on Gtk nativa (\texttt{lineal\_regresion\_guinativa.jl})}

\textit{Este programa fue escrito por Porfirio Sebasti\'an Rojas Ch\'avez.}
...
\textit{Este cuaderno fue generado con inteligencia artificial (Claude Sonnet~4.6, Anthropic).}
...
Wikipedia, ``Julia (lenguaje de programaci\'on),'' \textit{Wikipedia, La enciclopedia libre}, 2024. [En l\'inea]. Disponible en: \url{https://es.wikipedia.org/wiki/Julia_(lenguaje_de_programaci%C3%B3n)}

\bibitem{hastie2009elements}
Wikipedia, ``Descenso del gradiente,'' \textit{Wikipedia, La enciclopedia libre}, 2024. [En l\'inea]. Disponible en: \url{https://es.wikipedia.org/wiki/Descenso_del_gradiente}

\bibitem{goodfellow2016deep}
Wikipedia, ``Aprendizaje profundo,'' \textit{Wikipedia, La enciclopedia libre}, 2024. [En l\'inea]. Disponible en: \url{https://es.wikipedia.org/wiki/Aprendizaje_profundo}
